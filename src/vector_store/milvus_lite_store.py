"""
Milvus Lite Vector Store Module
--------------------------
Handles:
1. Connection setup to Milvus Lite (local file-based).
...
# ===============================================================
# 1️⃣ Milvus Lite Connection Setup
# ===============================================================
def connect_milvus():
    """
    Connects to Milvus Lite using a local database file.
    """
    try:
        client = MilvusClient(settings.MILVUS_DB_PATH)
        print(f"✅ Connected to Milvus Lite at {settings.MILVUS_DB_PATH}")
        return client
    except Exception as e:
        raise ConnectionError(f"❌ Failed to connect to Milvus Lite: {str(e)}")
...
def load_collection(client: MilvusClient, collection_name: str):
    ...
    Note: Milvus Lite automatically loads collections, but we keep this for compatibility.
    """
    try:
        # Milvus Lite loads collections automatically, but we can still call this
        client.load_collection(collection_name=collection_name)
        print(f"✅ Loaded collection '{collection_name}' into memory.")
    except Exception as e:
        # In Milvus Lite, this might not be necessary, so we just log
        print(f"ℹ️ Collection load call (may be automatic in Milvus Lite): {str(e)}")
